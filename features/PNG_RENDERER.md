...
Behavior
Implement renderPng(svgString, options) that returns a Promise resolving to a Buffer or Uint8Array containing PNG bytes. Options may include width, height, background, and density. The implementation should:
...
API
Export a named async function renderPng from src/lib/main.js. The function must accept (svgString, options) and return a Buffer or Uint8Array containing the PNG file bytes.
...
- Calling renderPng with a valid simple SVG string (for example a 1x1 svg with a single rect) returns a Buffer or Uint8Array whose first eight bytes equal the PNG signature: 89 50 4E 47 0D 0A 1A 0A.
