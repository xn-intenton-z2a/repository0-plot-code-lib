...
// Updated web tests to reflect demo controls
...
describe('Website', () => {
  ...
  test('index.html contains demo controls', () => {
    const html = readFileSync('src/web/index.html', 'utf8');
    expect(html).toContain('expr-input');
    expect(html).toContain('range-input');
    expect(html).toContain('plot-btn');
    expect(html).toContain('plot-wrapper');
    expect(html).toContain('points-count');
  });
});
