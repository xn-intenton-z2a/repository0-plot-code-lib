...
describe('Additional helper functions', () => {
  ...
  test('plotInverse returns valid inverse values and handles zero correctly', () => {
    const points = plotInverse(-1, 1, 10);
    expect(points.length).toBe(11);
    const zeroPoint = points.find(p => Math.abs(p.x) < 1e-10);
    expect(zeroPoint.y).toBeNull();
  });

  test('cumulativeSum returns correct cumulative sums', () => {
    const data = [1, 2, 3, 4];
  ...
  test('plotDampedOscillation returns damped oscillation values', () => {
    // Updated test call: provide 7 arguments: xMin is 0, xMax is 10, steps = 10
    const points = plotDampedOscillation(1, 0.1, 2, 0, 0, 10, 10);
    expect(points.length).toBe(11);
