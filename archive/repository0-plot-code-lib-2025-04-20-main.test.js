...
describe("CLI Options Parsing", () => {
  test("should correctly parse --expression, --range, and --file options when file is not .svg, .png, .json, .csv, or .xml", async () => {
    ...
    expect(errorSpy).toHaveBeenCalledWith(
      "Error: Only .svg, .png, .pdf, .json, .xml, and .csv files are supported for plot generation.",
    );
...
describe("Mutually Exclusive Input Options", () => {
  test("should output error when both --csv and --expression/--range options are provided", async () => {
    ...
    await main(args);
    expect(errorSpy).toHaveBeenCalledWith(
      "Error: Cannot use --csv together with --expression/--range. Please provide only one input mode.",
    );
...
describe("Tooltip Marker Triangle Option", () => {
  ...
  test("should render triangle tooltip markers for CSV based plot when --tooltip-shape triangle is provided", async () => {
    ...
    const args = [
      ...
      csvData,
      "--file",
      "tooltip_csv_triangle_output.svg",
      "--tooltip",
      "--tooltip-shape",
      "triangle",
    ];
