...
export function generateInteractivePlot(expression, start, end, step, fallbackMessage, logScaleX = false, logScaleY = false, svgWidth = 500, svgHeight = 300) {
  ...
  // Build polyline points and interactive circles with event handlers
  ...
  // Generate circle elements for each point with interactive event handlers for tooltips
...
export function generateInteractiveMultiPlot(expressions, start, end, step, fallbackMessage, logScaleX = false, logScaleY = false, svgWidth = 500, svgHeight = 300) {
  ...
  series.forEach((serie, index) => {
    if (serie.points.length > 0) {
      ...
      // Non-interactive circles without event handlers
      const circles = svgPoints.map(p => `<circle cx="${p.scaledX}" cy="${p.scaledY}" r="3" fill="${color}" />`).join("\n");
  ...
  series.forEach((serie, index) => {
    ...
    legend += `<text x="${legendX + 15}" y="${legendY - 2}" font-size="10" fill="black">${serie.expression}</text>`;
  });
  legend += `\n</g>\n`;
...
/**
 * Generates a non-interactive SVG multi-plot for multiple mathematical expressions. Each expression is plotted with distinct colors,
 * and the SVG includes enhanced axes, grid lines, tick marks, and a legend.
 * 
 * @param {string[]} expressions - Array of math expressions.
 * @param {number} start - Starting x value.
 * @param {number} end - Ending x value.
 ...
 * @param {number} [svgWidth=500] - SVG width.
 * @param {number} [svgHeight=300] - SVG height.
 * @returns {string} - SVG multi-plot string.
 */
