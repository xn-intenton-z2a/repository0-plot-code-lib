...
function throwInvalidNumberError(index, rawValue, evaluated, extraInfo = '') {
  ...
  const err = new Error(`Parameter ${index} error: The expression '${trimmedValue}' evaluated to ${evaluated}, which is not a finite number. Please ensure you provide a valid finite number (e.g., 0). Raw input was '${rawValue}'.`);
  err.code = 1;
  err.diagnostic = {
    index,
    rawValue,
