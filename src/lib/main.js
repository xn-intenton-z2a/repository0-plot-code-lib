...
Options:
  ...
  --file               The output filename for the generated plot. 
                       - If the file has a .svg extension, an SVG plot will be generated.
                       - If the file has a .png extension, a PNG plot will be generated by converting the SVG output.
                       - If the file has a .pdf extension, a PDF file will be generated with the plot image embedded.
                       - If the file has a .json extension, the computed plot data is exported as JSON.
                       - If the file has a .csv extension, the computed plot data is exported as CSV with a header row.
                       - If the file has a .xml extension, an XML document containing the computed plot data is generated.
  --stroke-color       (Optional) Custom stroke color for the plot's polyline.
  --stroke-width       (Optional) Custom stroke width for the plot's polyline.
  --width              (Optional) Custom width for the output. Defaults to 300.
  --height             (Optional) Custom height for the output. Defaults to 150.
  --grid               (Optional) Include grid lines in the plot.
  --log-scale          (Optional) Apply logarithmic scaling on the y-axis (requires positive y values).
  --background-color   (Optional) Set a custom background color.
  --title              (Optional) Set a title for the plot.
  --x-label            (Optional) Set a label for the x-axis.
  --y-label            (Optional) Set a label for the y-axis.
  --tooltip            (Optional) Add tooltips to each data point.
  --tooltip-format     (Optional) Customize tooltip text using {x} and {y} placeholders.
  --dash-array         (Optional) Custom dash pattern for the polyline (e.g., "5,5").
  --tooltip-style      (Optional) Custom CSS styling for tooltip markers.
  --x-tick-format      (Optional) Customize x-axis tick labels (use {value} placeholder).
  --y-tick-format      (Optional) Customize y-axis tick labels (use {value} placeholder).
  --font-family        (Optional) Set a custom font family for all text (e.g., "Arial, sans-serif").
  --minify             (Optional) Minify the SVG output by removing whitespace.
  ...
  if (options.file) {
    ...
    if (options.file.endsWith(".svg")) {
      ...
      try {
        ...
        console.log(`XML data file created at: ${options.file}`);
      } catch (error) {
