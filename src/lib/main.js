    ...
    if (options.file) {
      ...
      for (let i = 0; i < n; i++) {
        ...
        try {
          y = f(x);
          // NaN Handling:
          // Check if the result is not a valid number. If y is not a number or is NaN,
          // replace it with null to ensure that the JSON output remains valid since JSON does not support NaN values.
