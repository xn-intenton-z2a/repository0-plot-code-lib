...
function throwInvalidNumberError(index, rawValue, evaluated, extraInfo = '') {
  ...
  const err = new Error(`Parameter ${index} error: The expression '${trimmedValue}' evaluated to ${evaluated}, which is not a finite number. Please replace any instance of literal 'NaN' (in any casing or extra whitespace) or any non-finite numeric expression with a valid finite number (e.g., 0). Raw input was '${rawValue}'.`);
  err.code = 1;
  err.diagnostic = {
    index,
    rawValue,
    trimmedValue,
    evaluated,
    suggestion: "Replace any instance of literal 'NaN' or non-finite expressions with a valid finite number, for example 0."
  };
  throw err;
}
...
function evaluateParameter(p, index) {
  ...
  // Reject if the trimmed value is exactly 'NaN' regardless of casing
  ...
  // Check: if evaluated result is not a finite number or is NaN (e.g., expressions like '2+NaN'), throw error
