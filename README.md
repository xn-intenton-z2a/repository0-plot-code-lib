...
- New Feature: **Custom Tooltip Marker Shape Option**: You can now choose the shape of the data point markers when tooltips are enabled using the `--tooltip-shape` option. **Accepted values are "circle" (default) and "square".** If an invalid value is provided, the CLI will output an error and halt execution.
...
- `--tooltip-shape`: (Optional) Set the tooltip marker shape. **Accepted values: "circle" (default) or "square".** If an invalid value is provided, the program outputs an error and halts execution.
